...
export default class ClientSocket {
    ...
    public constructor(config: any,client:string = 'wsclient') {
        this.config = config;
        this.client = client;
    }
    ...
    getSocket() {
        ...
        this.socket.on('connect', () => {
            ...
            this.socket.on('vp1_current', current => {               
                this.emitEvent('vp1_current', current);
            });

            this.socket.on('vp1_hilows', hilows => {
                this.emitEvent('vp1_hilows', hilows);
            });
